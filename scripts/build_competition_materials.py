...
PROJECT = {
    "name": "CoursePilot Agent 药学管理课程智能体",
    "track": "课程智能体",
    "major": "医学",
    "specialty": "药学 / 药事管理 / 药学管理",
...
FULL_DESCRIPTION = """
一、开发背景与拟解决问题
CoursePilot Agent 面向药学管理与药事服务相关课程，聚焦新教师首次授课和课程持续建设中的真实痛点。药学类课程中的管理学内容兼具专业性、政策性和实践性，教师既要讲清管理学概念，又要把概念落到药品经营、医院药学、医保支付、处方流转、慢病管理、患者教育和用药安全等真实场景。新教师常见困难包括：课程目标难以对齐培养方案，教材内容不会取舍，抽象理论难以转化为药学案例，学情预判不足，课堂互动和节奏控制不稳定，过程性评价与作业反馈缺少标准，课后复盘和课程资产沉淀不足。以上问题会导致课堂成为泛管理学讲授，学生知道概念却不知道如何在药学岗位情境中判断、举证和决策。

二、智能体定位与应用载体
本智能体申报赛道为“课程智能体”，定位为药学管理课程的教学工作台。它不是通用问答工具，而是围绕一门课程、一类教师和一组典型教学任务进行设计：以培养方案、课程大纲、教材章节、班级学情、泛雅课程上下文和教师输入主题为起点，辅助教师完成“课前诊断、课中设计、课后评价、持续改进”的全流程工作。当前应用载体包括网页端原型、本次课运行中心、教学痛点场景库、教学流程图、泛雅第三方入口参数识别和本地后端 Connector。教师可通过浏览器访问系统，也可将部署后的公网 HTTPS 地址作为泛雅第三方链接接入课程。
...
五、实施路径
项目已完成新教师痛点清单整理、四阶段教学流程梳理、CoursePilot 网页端原型、本次课运行中心、评审演示路径、泛雅第三方链接接入说明和本地 Connector 开发。以“药学管理中的 SWOT 分析：连锁药店慢病服务决策”为例，教师输入课程主题后，系统可生成课次目标、药学案例导入、课堂提问、Rubric、泛雅发布稿和课后复盘要点。正式参赛前，建议完成公网部署、测试账号配置、1-2 个真实班级或课程组的试运行记录，并补充备课时间变化、学生参与情况、作业质量或教师满意度等量化数据。

六、创新点
第一，课程专属性强。系统围绕药学管理课程构建知识与任务，不停留在通用聊天，而是把医学教育场景、课程目标、药学岗位能力和平台执行动作绑定起来。第二，真实问题导向。系统内置二十类新教师痛点，覆盖课前、课中、课后和课程建设全过程，使智能体服务真实教学流程。第三，药学场景深度融合。系统将管理学抽象概念转化为药事服务、医保政策、药品经营和患者安全案例，提升课程的医学专业性和岗位适配度。第四，闭环落地能力突出。系统从目标、内容、互动、评价到复盘形成链路，并通过泛雅入口把 AI 生成内容转化为教学平台中的任务、讨论和作业。第五，安全可控。所有正式发布动作均保留教师确认，敏感配置存放于后端环境变量，评审材料可生成匿名版本，降低数据和身份泄露风险。

七、应用成效与推广价值
当前项目已形成可演示、可部署、可迭代的课程智能体原型，沉淀了 20 个教学痛点场景、4 个教学阶段、1 套泛雅接入流程和 1 条 3 分钟评审体验路径。原型验证表明，系统能够把一次药学管理课程从“教师想讲什么”转化为“学生完成什么任务、教师获得什么学习证据、课程平台沉淀什么数据”的结构化方案。对新教师而言，系统降低了备课启动成本，减少材料拼接和口径不一致；对学生而言，药学案例、课堂讨论和 Rubric 让学习任务更清楚；对课程团队而言，系统可沉淀统一标准、共享案例和跨轮次改进记录。后续可推广至药事管理、临床药学服务、医药市场营销、医院药学管理等课程，也可作为医学类课程智能体建设的模板。
...
INTRO_1200 = """
CoursePilot Agent 药学管理课程智能体面向药学管理与药事服务相关课程，针对新教师首次授课中目标不清、案例转化难、互动评价弱、平台沉淀不足等问题，构建贯穿课前诊断、课中设计、课后评价、持续改进的课程智能体。系统以培养方案、课程大纲、教材章节、班级学情、泛雅课程上下文和教师输入主题为起点，将课程目标拆解为“课程-章节-课次”三级目标，将管理学概念映射到连锁药店慢病服务、医院药房处方审核、医保支付、药品经营与用药安全等药学情境，自动生成教案、PPT 提纲、案例导入脚本、分层提问、Rubric、作业说明和复盘建议。
...
EFFECT_800 = """
项目已形成可运行原型和完整评审演示路径，沉淀 20 个新教师教学痛点、4 个教学阶段、1 套泛雅接入流程和 1 个本次课运行中心。以“药学管理中的 SWOT 分析：连锁药店慢病服务决策”为例，系统可在一次操作中输出课次目标、药学案例导入、课堂提问、课堂节奏、Rubric、泛雅发布稿和复盘要点，帮助教师从“准备材料”转向“设计学习任务和评价证据”。
...
VIDEO_ROWS = [
    ...
    ("0:35-1:25", "应用场景说明", "打开 CoursePilot 首页，说明面向药学管理与药事服务课程，服务课前、课中、课后和持续改进闭环。", "突出课程智能体，而非通用问答。"),
    ("1:25-2:20", "输入课程主题", "输入“药学管理中的 SWOT 分析：连锁药店慢病服务决策”，展示系统识别药学情境和教学阶段。", "体现课程上下文和药学场景转化。"),
...
TECH_SECTIONS = [
    ("一、原创主体与版本说明", [
        ...
        "项目名称：CoursePilot Agent 药学管理课程智能体。",
...
def build_checklist() -> Path:
    ...
    add_bullets(doc, [
        "视频建议命名为：CoursePilot Agent 药学管理课程智能体.mp4。",
...
def build_application_form() -> Path:
    ...
    add_kv_table(doc, [
        ("学校/医院教务部门意见（盖章）", "该智能体围绕药学管理课程教学真实问题开展设计，具有课程建设价值和推广潜力。建议推荐参赛。\n\n年    月    日"),
...
def build_video_script() -> Path:
    ...
    add_bullets(doc, [
        ...
        "建议演示主题：药学管理中的 SWOT 分析：连锁药店慢病服务决策。",
    ...
    doc.add_paragraph(
        "本项目是 CoursePilot Agent 药学管理课程智能体，面向药学管理与药事服务相关课程的新教师，"
        "解决首次授课中目标不清、案例转化难、互动评价弱和平台沉淀不足的问题。下面将以"
        "“药学管理中的 SWOT 分析：连锁药店慢病服务决策”为例，展示智能体如何把课程主题转化为"
