...
slides = [
    {
        ...
        "narration": "本项目是 CoursePilot Agent 药学管理课程智能体，申报课程智能体赛组。它面向药学管理与药事服务相关课程的新教师，解决第一次授课中目标不清、案例转化难、互动评价弱和平台沉淀不足的问题。下面的展示全程使用匿名画面，不出现参赛者姓名、学校和院系信息。",
        "bullets": ["申报赛道：课程智能体", "应用课程：药学管理 / 药事服务相关课程", "演示主题：连锁药店慢病服务决策中的 SWOT 分析"],
    },
    {
        "title": "真实痛点：新教师不是缺材料",
    ...
    },
    {
        ...
        "narration": "本次演示以药学管理中的 SWOT 分析为例。教师输入连锁药店慢病服务决策后，智能体会识别药学情境，关联药品经营、患者教育、医保支付和用药安全约束，再把抽象管理学工具转化为学生可以讨论和提交的学习任务。",
        "bullets": ["课程主题：药学管理中的 SWOT 分析", "药学情境：连锁药店慢病服务、患者教育、医保支付、门店运营", "教学目标：让学生能用证据分析药事服务方案的优势、风险和合规边界"],
    },
    {
    ...
    {
        "title": "应用成效与推广价值",
        "narration": "目前项目已经形成可运行原型，沉淀二十个新教师痛点、四个教学阶段、一套泛雅接入流程和一条评审体验路径。它可以帮助新教师降低备课启动难度，让学生获得更清楚的药学情境任务，也让课程团队沉淀统一标准、共享案例和跨轮次复盘记录。后续可推广到药事管理、临床药学服务、医药市场营销和医院药学管理等课程。",
...
def compose_video(slide_paths, audio_paths, srt_path: Path):
    ...
    out_mp4 = OUT / "CoursePilot_Agent_药学管理课程智能体_展示视频初稿.mp4"
