...
function renderVisualCard(card, index) {
  ...
  return `<button class="chart-panel visual-card ${wide}" type="button" data-insight="${esc(card.insight)}">
    <div class="visual-card-head">
      ...
      <span class="click-chip">可点击</span>
    </div>
...
function svgWrap(content, viewBox = "0 0 420 190", extra = "") {
  return `<svg viewBox="${viewBox}" class="chart-svg ${extra}" role="img">${content}</svg>`;
}
...
function histogram() {
  return barChart([3, 6, 8, 10, 7, 4], ["0-39", "40-49", "50-59", "60-69", "70-79", "80+"], [PALETTE.rose, PALETTE.rose, PALETTE.gold, PALETTE.gold, PALETTE.blue, PALETTE.sage]);
}
...
function trendChart() {
  ...
  const grid = [0, 25, 50, 75, 100].map((value) => {
    ...
    return `<line x1="${m.l}" x2="${w - m.r}" y1="${y}" y2="${y}" stroke="rgba(31,30,29,.08)"></line><text x="${m.l - 8}" y="${y + 4}" text-anchor="end" font-size="10" fill="${PALETTE.muted}">${value}</text>`;
  }).join("");
  const lines = series.map((item, index) => {
    ...
    return `<path d="M ${pts.map((point) => point.join(" ")).join(" L ")}" fill="none" stroke="${item.color}" stroke-width="2.5"></path>
      <text x="${w - 48}" y="${22 + index * 16}" font-size="10" fill="${item.color}">${item.label}</text>
      ${pts.map((point) => `<circle cx="${point[0]}" cy="${point[1]}" r="4" fill="${item.color}"></circle>`).join("")}`;
  }).join("");
  const labels = TREND_DATA.map((data, index) => `<text x="${m.l + iw * index / (TREND_DATA.length - 1)}" y="${h - 8}" text-anchor="middle" font-size="10" fill="${PALETTE.muted}">${data.label}</text>`).join("");
  return svgWrap(grid + lines + labels);
}
...
function scatterPlot(points, xLabel, yLabel) {
  ...
  const grid = [0, 25, 50, 75, 100].map((value) => {
    ...
    const y = m.t + ih - ih * value / 100;
    return `<line x1="${m.l}" x2="${w - m.r}" y1="${y}" y2="${y}" stroke="rgba(31,30,29,.07)"></line><line x1="${x}" x2="${x}" y1="${m.t}" y2="${m.t + ih}" stroke="rgba(31,30,29,.05)"></line>`;
  }).join("");
  ...
  return svgWrap(`${grid}${dots}<text x="${m.l + iw / 2}" y="${h - 4}" text-anchor="middle" font-size="10" fill="${PALETTE.muted}">${esc(xLabel)}</text><text x="12" y="${m.t + ih / 2}" text-anchor="middle" font-size="10" fill="${PALETTE.muted}" transform="rotate(-90 12 ${m.t + ih / 2})">${esc(yLabel)}</text>`);
}

function horizontalBars(data) {
  const max = Math.max(...data.map((item) => item.value), 1);
  const colors = [PALETTE.accent, PALETTE.blue, PALETTE.gold, PALETTE.sage, PALETTE.rose];
  return `<div class="bars">${data.map((item, index) => `<div><div class="bar-label"><span>${esc(item.label)}</span><span>${item.value}</span></div><div class="bar-track"><i style="width:${item.value / max * 100}%;background:${colors[index % colors.length]}"></i></div></div>`).join("")}</div>`;
}

function barChart(data, labels, colors = []) {
  const w = 420;
  const h = 190;
  const m = { l: 24, r: 12, t: 12, b: 34 };
  const iw = w - m.l - m.r;
  const ih = h - m.t - m.b;
  ...
  const grid = [0, .25, .5, .75, 1].map((percent) => {
    ...
    return `<line x1="${m.l}" x2="${w - m.r}" y1="${y}" y2="${y}" stroke="rgba(31,30,29,.08)"></line>`;
  }).join("");
  const bars = data.map((value, index) => {
    ...
    return `<rect x="${x}" y="${y}" width="${bw}" height="${bh}" rx="7" fill="${color}" opacity=".86"></rect><text x="${x + bw / 2}" y="${h - 10}" text-anchor="middle" font-size="10" fill="${PALETTE.muted}">${esc(labels[index])}</text>`;
  }).join("");
  return svgWrap(grid + bars);
}
...
function heatmap(labels) {
  ...
  return `<div class="heatmap" style="--cols:${cols.length}"><div></div>${cols.map((col) => `<b>${esc(col)}</b>`).join("")}${labels.map((row, rowIndex) => `<span>${esc(row)}</span>${cols.map((col, colIndex) => {
    const value = (rowIndex * 23 + colIndex * 17 + 42) % 100;
    return `<i style="background:rgba(217,119,87,${0.08 + value / 150})">${value}</i>`;
  }).join("")}`).join("")}</div>`;
}
...
function donut(value, label) {
  ...
  return svgWrap(`<circle cx="90" cy="90" r="${r}" fill="none" stroke="rgba(31,30,29,.08)" stroke-width="16"></circle><circle cx="90" cy="90" r="${r}" fill="none" stroke="${PALETTE.accent}" stroke-width="16" stroke-linecap="round" stroke-dasharray="${circumference}" stroke-dashoffset="${offset}" transform="rotate(-90 90 90)"></circle><text x="90" y="86" text-anchor="middle" font-size="28" font-weight="700" fill="${PALETTE.accentDark}">${value}%</text><text x="90" y="110" text-anchor="middle" font-size="12" fill="${PALETTE.muted}">${esc(label)}</text>`, "0 0 180 180", "donut-svg");
}
