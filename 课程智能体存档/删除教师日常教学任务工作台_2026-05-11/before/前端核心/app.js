...
const HOME_TASK_PANEL_CONTENT = {
  navigation: {
    kicker: "Teaching navigation",
    title: "生成教学导航",
    summary: "把 SWOT 分析课拆成课前、课中、课后三阶段，并保留 20 个后台诊断检查点。",
    status: "待教师审校",
  ...
  resources: {
    kicker: "Resource intake",
    title: "整理课程资源",
    summary: "把课程大纲、案例材料和泛雅模拟学情数据整理成可调用的备课上下文。",
    status: "模拟数据边界",
    flow: ["解析课程大纲", "归并案例证据", "标记泛雅模拟数据", "形成资源索引"],
    outputTitle: "课程资源索引",
    outputText: "输出知识点、案例事实、学生基础差异和课堂风险提示，供教师确认后继续生成材料。",
    actionLabel: "进入教学实践",
  ...
  review: {
    kicker: "Teaching review",
    title: "生成复盘报告",
    summary: "把课堂表现、即时测评和作业样例转化为本节课的教学反思与下轮改进建议。",
    status: "教师确认后保存",
    flow: ["导入课堂表现", "识别高频误区", "汇总学习证据", "生成改进建议"],
    outputTitle: "复盘报告草稿",
    outputText: "输出学生常见误区、活动参与情况、反馈闭环完整度和下一轮课堂调整建议。",
    actionLabel: "进入教学实践",
    actionHref: "./practice.html",
  },
  assets: {
...
function initHomeTaskTabs() {
  const tabs = $$(".home-task-tab");
  const title = $("#homeTaskTitle");
  const summary = $("#homeTaskSummary");
  if (!tabs.length || !title || !summary) return;

  const kicker = $("#homeTaskKicker");
  const status = $("#homeTaskStatus");
  const flow = $("#homeTaskFlow");
  const outputTitle = $("#homeTaskOutputTitle");
  const outputText = $("#homeTaskOutputText");
  const primaryAction = $("#homeTaskPrimaryAction");

  function renderHomeTask(taskId) {
    const task = HOME_TASK_PANEL_CONTENT[taskId] || HOME_TASK_PANEL_CONTENT.navigation;
    tabs.forEach((tab) => {
      const isActive = tab.dataset.homeTask === taskId;
      tab.classList.toggle("is-active", isActive);
      tab.setAttribute("aria-pressed", String(isActive));
    });
    if (kicker) kicker.textContent = task.kicker;
    title.textContent = task.title;
    summary.textContent = task.summary;
    if (status) status.textContent = task.status;
    if (flow) {
      flow.innerHTML = task.flow.map((item) => `<span>${item}</span>`).join("");
    }
    if (outputTitle) outputTitle.textContent = task.outputTitle;
    if (outputText) outputText.textContent = task.outputText;
    if (primaryAction) {
      primaryAction.textContent = task.actionLabel;
      primaryAction.href = task.actionHref;
    }
  }

  tabs.forEach((tab) => {
    tab.setAttribute("aria-pressed", String(tab.classList.contains("is-active")));
    tab.addEventListener("click", () => renderHomeTask(tab.dataset.homeTask));
  });
}
