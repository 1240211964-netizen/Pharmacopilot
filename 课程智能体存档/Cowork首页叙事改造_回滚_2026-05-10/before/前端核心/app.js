...
const HOME_COWORK_TASKS = {
  navigation: {
    status: "已实现路径",
    title: "生成教学导航",
    copy: "把课程目标转化为课前、课中、课后三阶段教学路径。",
    input: "课程大纲、SWOT 案例材料、教师设定的学习目标。",
    process: "生成三阶段教学导航，并保留 20 环节后台诊断。",
    output: "教学导航、环节 Rubric、训练报告草稿。",
    primaryText: "进入教学导航",
    primaryHref: "./teaching-navigation.html",
    secondaryText: "进入教学实践",
    secondaryHref: "./practice.html",
  },
  resources: {
    status: "模拟中",
    title: "整理课程资源",
    copy: "把大纲、案例和泛雅模拟数据整理为可引用材料。",
    input: "课程大纲、SWOT 案例包、泛雅模拟导出的作业与讨论记录。",
    process: "标注材料来源边界，整理案例事实、学生基础和课堂风险。",
    output: "课程资源索引、案例证据表、课前学情摘要。",
    primaryText: "进入教学实践",
    primaryHref: "./practice.html",
    secondaryText: "查看教学资产",
    secondaryHref: "./assets.html",
  },
  rubric: {
    status: "已实现路径",
    title: "配置评价量规",
    copy: "为课堂任务生成 Rubric、证据要求和反馈建议。",
    input: "教学目标、学生产出要求、课堂活动与学习证据清单。",
    process: "对齐目标、活动和评价证据，生成可审校的 Rubric 维度。",
    output: "评价量规、低分维度诊断、教师反馈建议。",
    primaryText: "进入教学导航",
    primaryHref: "./teaching-navigation.html",
    secondaryText: "查看教学资产",
    secondaryHref: "./assets.html",
  ...
  review: {
    status: "模拟中",
    title: "生成复盘报告",
    copy: "把学生表现和课堂数据转化为下一轮改课建议。",
    input: "课堂表现、作业结果、测验记录和教师观察。",
    process: "诊断学习困难，汇总共性问题、个体差异和补救支持建议。",
    output: "教学反思、学习数据分析、下一轮持续改进计划。",
    primaryText: "进入教学实践",
    primaryHref: "./practice.html",
    secondaryText: "保存到资产",
    secondaryHref: "./assets.html",
  ...
  assets: {
    status: "已实现路径",
    title: "沉淀教学资产",
    copy: "把教案、Rubric、反馈语和学生样例归档复用。",
    input: "已确认的教学导航、实践方案、Rubric 和复盘记录。",
    process: "保留来源边界、复用动作和教师审校状态。",
    output: "可检索、可复用、可继续生成的教学资产。",
    primaryText: "查看教学资产",
...
function initHomeCoworkTasks() {
  const tabs = $$("[data-cowork-task]");
  if (!tabs.length) return;
  tabs.forEach((tab) => {
    tab.addEventListener("click", () => renderHomeCoworkTask(tab.dataset.coworkTask));
  });
  const active = tabs.find((tab) => tab.classList.contains("is-active"))?.dataset.coworkTask || tabs[0]?.dataset.coworkTask;
  renderHomeCoworkTask(active);
}

function renderHomeCoworkTask(taskKey) {
  const task = HOME_COWORK_TASKS[taskKey] || HOME_COWORK_TASKS.navigation;
  const setText = (selector, value) => {
    const target = $(selector);
    if (target) target.textContent = value;
  };
  setText("#coworkTaskStatus", task.status);
  setText("#coworkTaskTitle", task.title);
  setText("#coworkTaskCopy", task.copy);
  setText("#coworkTaskInput", task.input);
  setText("#coworkTaskProcess", task.process);
  setText("#coworkTaskOutput", task.output);
  const primary = $("#coworkTaskPrimaryLink");
  if (primary) {
    primary.textContent = task.primaryText;
    primary.href = task.primaryHref;
  }
  const secondary = $("#coworkTaskSecondaryLink");
  if (secondary) {
    secondary.textContent = task.secondaryText;
    secondary.href = task.secondaryHref;
  }
  $$("[data-cowork-task]").forEach((tab) => {
    const isActive = tab.dataset.coworkTask === taskKey;
