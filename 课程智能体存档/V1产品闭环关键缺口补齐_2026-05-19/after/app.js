...
function renderHomePracticePreview(activeAttachment) {
  const metrics = [
    ["泛雅模拟", "已授权", "测试账号与数据边界已确认"],
    ["课程资源", "4 类", "课件、案例、作业、讨论记录"],
    ["班级学情", "72%", "预习完成率进入 20 环节判断"],
  ];
  const activities = [
    ["选择真实课程", "教师确认课程、班级、资源和学习过程数据范围。"],
    ["真实课程 20 环节实践", "沿用教学导航训练框架，叠加泛雅模拟数据增强判断。"],
    ["生成真实课程教学实践方案", "输出课堂任务、评价量规、复盘建议和平台复制文本。"],
  ];
  const rubric = [
    ["生成方案", "82%"],
    ["复制到泛雅", "68%"],
  ...
  return `
    ...
    <div class="home-output-rubric" aria-label="实践方案动作预览">
      ${rubric
        .map(
          ([label, width]) => `
            <div>
...
const routeTrainingNodes = [
  ...
  },
  {
    ...
    output: "教学目标与预期学习成果表。",
    rubric: "目标是否具体、可观察、可评价，并能体现药事管理课程特色。",
...
function ensureTeachingNavigationBaseline() {
  const hasStoredProgress =
    Number(trainingState.currentStepId || 1) > 1 ||
    (trainingState.completedStepIds || []).length > 0 ||
    Object.keys(trainingState.stepResults || {}).length > 0;
  if (hasStoredProgress) return;
  const completedIds = routeTrainingNodes.filter((node) => node.status === "completed").map((node) => node.id);
  const currentNode = routeTrainingNodes.find((node) => node.status === "current") || routeTrainingNodes[0];
  trainingState.completedStepIds = completedIds;
  trainingState.currentStepId = currentNode.id;
  saveTrainingState();
}
...
function initTeachingNavigationPage() {
  ...
  loadTrainingState();
  ensureTeachingNavigationBaseline();
...
function renderStageOverview() {
  ...
  container.innerHTML = routeStageDefinitions
    .map((stage) => {
      ...
      return `
        <button
          class="teaching-stage-card route-stage-pill route-stage-tab route-stage-tab-${stage.tone} stage-status-${status === "已完成" ? "complete" : status === "进行中" ? "current" : status === "可进入" ? "available" : "locked"} ${currentStageId === Number(stage.id) ? "is-current" : ""} ${selectedStageId === Number(stage.id) ? "is-selected" : ""}"
          type="button"
          data-route-stage-pill="${stage.id}"
          aria-label="查看${escapeHtml(stage.title)} ${escapeHtml(stage.range)}"
        >
          <span class="teaching-stage-index">${stage.id}</span>
          <div class="teaching-stage-body">
            <h3>${escapeHtml(stage.title)}</h3>
            <p>${escapeHtml(stage.range)}</p>
          </div>
          <strong class="teaching-stage-meta">${completed}/${nodes.length}</strong>
        </button>
      `;
    })
    .join("");
...
function focusRouteNodeDetailPanel() {
  requestAnimationFrame(() => {
    const detail = $("#teachingNodeDetail");
    const panel = detail?.closest?.(".route-detail-card");
  ...
  return `
    ...
    >
      <span class="route-level-shell">
        <span class="route-level-number">关卡 ${stepLabel}</span>
      ...
      <span class="route-node-tooltip" role="tooltip">
        ...
        <em>${escapeHtml(statusLabel)} · ${escapeHtml(mission.reward)} · ${mission.progress}%</em>
        <small>${escapeHtml(mission.challenge)}</small>
      </span>
    </button>
  `;
}
...
function renderRouteGameMission(node, stepId, status, options = {}) {
  ...
  const showProgress = status === "current" || status === "completed";
  return `
    <section class="route-game-mission route-mission-brief" aria-label="关卡任务简报">
      <div class="route-game-mission-head">
        ...
        <div>
          <p class="eyebrow">本关任务</p>
          <h4>${escapeHtml(mission.challenge)}</h4>
        </div>
        <strong>${escapeHtml(statusLabel)}</strong>
      </div>
      ${showProgress ? `<div class="route-game-meter-large" aria-label="关卡进度 ${mission.progress}%"><span style="width:${mission.progress}%"></span></div>` : ""}
      <div class="route-mission-lines">
        <p><span>通关条件</span>${escapeHtml(node.rubric)}</p>
        <small>解锁能力：${escapeHtml(mission.power)} · ${escapeHtml(node.output)}</small>
      </div>
      <details class="route-mission-more">
        <summary>查看输入与产出边界</summary>
        <dl>
          <div><dt>教师输入</dt><dd>${inputRequirements.map((item) => escapeHtml(item)).join("；")}</dd></div>
          <div><dt>智能体产出</dt><dd>${escapeHtml(node.output)}</dd></div>
          <div><dt>能力徽记</dt><dd>${escapeHtml(mission.reward)}</dd></div>
        </dl>
      </details>
...
function renderRouteStoryRows(node, step, stepId) {
  const storyRows = [
    {
      icon: "问",
      title: "本环节要解决的问题",
      description: node.goal || step.coreQuestion,
    },
    {
      icon: "入",
      title: "教师需要提供的信息",
      description: getRouteNodeInputRequirements(node).join("；"),
    },
    {
      icon: "出",
      title: "智能体生成结果",
  ...
  return storyRows
    .map(
      (item) => `
        <article class="route-story-row">
          <span class="route-story-icon" aria-hidden="true">${escapeHtml(item.icon)}</span>
          <span class="route-story-copy">
            <strong>${escapeHtml(item.title)}</strong>
            <p>${escapeHtml(item.description)}</p>
          </span>
          <span class="route-story-chevron" aria-hidden="true">›</span>
        </article>
      `,
    )
    .join("");
...
function renderNodeDetailPanel() {
  ...
  const commonHeader = `
      ...
      <header class="route-workbench-head">
        <h3>关卡 ${String(stepId).padStart(2, "0")}｜${escapeHtml(step.title)}</h3>
        <p>${escapeHtml(isTrainingMode ? "选择一个主方案，生成简短草稿后确认通关。" : node.goal)}</p>
    ...
    <article class="route-node-detail-card route-workbench-card route-node-expanded-card" data-route-workbench-step="${stepId}">
      ...
      <section class="route-workbench-block route-option-workbench" aria-labelledby="route-option-title-${stepId}">
        <div class="route-block-head">
          <div>
            <p class="eyebrow">Strategy cards</p>
            <h4 id="route-option-title-${stepId}">选择一个主方案</h4>
          </div>
          <span>${selectedOptionId ? `已选 ${escapeHtml(selectedOptionId)}` : "推荐 3 个"}</span>
        </div>
        ${renderTrainingOptionCards(step, choice, { compact: true, limit: 3 })}
      </section>

      ${
        selectedOptionId
          ? `
            <section class="route-workbench-block route-generated-workbench route-generated-compact" aria-labelledby="route-generated-title-${stepId}">
              <div class="route-block-head">
                <div>
                  <p class="eyebrow">Agent draft</p>
                  <h4 id="route-generated-title-${stepId}">生成摘要</h4>
                </div>
                <span>${escapeHtml(scoreMeta.label)}</span>
              </div>
              <p class="route-generated-summary">${escapeHtml(compactAssetGraphText(preview.fragment || "当前方案暂未生成草稿。", 118))}</p>
              <div class="route-detail-actions">
                <button class="primary-action" data-route-action="confirm" type="button">确认通关</button>
                <button class="secondary-action" data-route-action="collapse-card" type="button">返回简报</button>
              </div>
              <details class="route-training-more">
                <summary>展开智能草稿、成熟度评分和低分维度</summary>
          ...
          `
          : `<p class="route-generated-placeholder">选择一个方案后，系统会显示简短生成结果和通关按钮。</p>`
      }
    </article>
  `;
  $("[data-route-action='close-card']", container)?.addEventListener("click", closeRouteNodeCard);
...
function renderProgressSummary() {
  ...
  const completedCount = trainingState?.completedStepIds?.length || 0;
  const currentPhase = getTeachingPhaseTitle(getTeachingPhaseIdByNodeId(current.id));
  const nextLabel = `${String(current.id).padStart(2, "0")} ${current.title}`;
  ...
  container.innerHTML = `
    <article class="route-progress-strip">
      <div><span>已完成</span><strong>${completedCount} / ${trainingSteps.length}</strong></div>
      <div><span>当前阶段</span><strong>${escapeHtml(currentPhase)}</strong></div>
      <div><span>下一关</span><strong>${escapeHtml(nextLabel)}</strong></div>
      <details class="route-progress-details">
        <summary>查看完整复盘</summary>
        <div class="route-progress-detail-grid">
          ${summaries
            .map(
              (item) => `
            ...
            .join("")}
          <section class="route-progress-card route-next-advice">
            <span>下一步建议</span>
            <strong>${escapeHtml(nextLabel)}</strong>
            <p>建议继续挑战“${escapeHtml(current.title)}”，检查该环节是否同时回应 ${escapeHtml(currentPhase)}、学习证据和药事管理真实情境。</p>
          </section>
        </div>
      </details>
    </article>
...
function renderTrainingOptionCards(step, choice, options = {}) {
  const entries = Object.entries(step.options || {});
  const compact = Boolean(options.compact);
  const limit = Number(options.limit || entries.length);
  const visibleEntries = compact ? entries.slice(0, limit) : entries;
  const hiddenEntries = compact ? entries.slice(limit) : [];
  const renderCard = ([id, label], isHidden = false) => {
    const isPrimary = choice.primary === id;
    const isSecondary = choice.secondary.includes(id);
    const roleLabel = isPrimary ? "主方案" : isSecondary ? "辅助" : isHidden ? "更多" : "推荐";
    return `
      <article class="option-card ${compact ? "option-card-compact" : ""} ${isPrimary ? "primary-selected selected" : ""} ${isSecondary ? "secondary-selected auxiliary" : ""}" data-training-option-card="${id}">
        <div class="option-topline">
          <span class="option-id">${compact ? id : `Card ${id}`}</span>
        ...
        ${
          compact
            ? `<div class="option-actions option-actions-compact"><button class="${isPrimary ? "active" : ""}" data-primary-option="${id}" type="button">${isPrimary ? "已选" : "选择"}</button></div>`
  ...
  return `
    <div class="option-grid route-option-card-deck">
      ${visibleEntries.map((entry) => renderCard(entry)).join("")}
    </div>
    ${
      hiddenEntries.length
        ? `<details class="route-more-options">
            <summary>更多方案</summary>
            <div class="option-grid route-option-card-deck route-option-card-deck-more">
              ${hiddenEntries.map((entry) => renderCard(entry, true)).join("")}
            </div>
          </details>`
        : ""
    }
...
const MANAGEMENT_COURSE_GRAPH_NODES = [
  {
    ...
    x: 50,
    y: 50,
    summary: "以管理职能和课程证据链组织教学资产，帮助教师从课程结构进入模块资源、评价材料和复用动作。",
    boundary: ["课程主干是管理学原理，不把单个案例工具提升为课程默认主题。", "药事管理语境用于案例化表达，正式使用前需要教师确认案例边界和课程资料来源。"],
    quality: ["课程模块完整度优先于单点案例丰富度。", "资产进入复用前必须保留来源边界、使用限制和教师确认状态。"],
    actions: ["查看模块覆盖", "汇总资产缺口", "复制课程图谱说明"],
  },
  {
    ...
    x: 27,
    y: 24,
    summary: "建立管理学基本概念、组织环境和药事管理情境之间的共同语言。",
    boundary: ["聚焦管理者角色、组织环境、利益相关者和课程案例事实。", "不直接下沉到单一分析工具，先确认学生的概念基础和案例阅读边界。"],
    quality: ["需要课程资料、案例背景和学情分析证据互相对应。", "诊断重点是概念误解和情境迁移困难。"],
    actions: ["生成课前诊断问题", "整理概念对照表", "标注案例事实来源"],
  },
  {
    ...
    x: 70,
    y: 24,
    summary: "围绕目标设定、方案比较、决策依据和风险边界组织教学资源。",
    boundary: ["SWOT、决策矩阵和风险清单都只是本模块下的工具示例。", "教学资产应回到计划与决策能力，不把 SWOT 泛化为整门课或整页资产主题。"],
    quality: ["需要能说明学生如何从证据进入方案判断。", "评价材料要区分工具填写质量和管理决策质量。"],
    actions: ["生成决策任务单", "校准方案比较维度", "检查 SWOT 示例边界"],
  },
  {
    id: "swot-example",
    type: "example",
    ...
    x: 82,
    y: 38,
    summary: "仅作为计划与决策模块中的案例工具，服务学生识别内外部因素并形成有证据支撑的管理建议。",
    boundary: ["SWOT 不代表教学实践页或教学资产页的默认主题。", "示例必须绑定具体课程案例、政策材料和教师确认的评价标准。"],
    quality: ["关注因素分类、证据引用、策略匹配和风险意识。", "正式评分前需要教师调整权重与学校过程性评价规范。"],
    actions: ["生成 SWOT 课堂任务", "生成评价量规", "复制示例边界声明"],
  },
  {
    ...
    x: 20,
    y: 56,
    summary: "围绕组织结构、岗位分工、流程协同和药事管理组织场景组织资源。",
    boundary: ["只处理课程案例中的组织设计问题，不替代真实机构人事决策。", "涉及岗位职责和制度文本时必须标注材料来源。"],
    quality: ["需要能支撑角色分工、流程节点和协作机制。", "课堂活动应能观察学生是否理解结构与任务之间的关系。"],
    actions: ["生成岗位分工案例", "创建小组协作脚本", "整理流程图证据"],
  },
  {
    ...
    x: 36,
    y: 76,
    summary: "沉淀领导方式、沟通情境、激励机制和团队反馈类教学资产。",
    boundary: ["课堂材料以教学案例为限，不对真实个人或学生进行心理判断。", "评价学生表达时优先使用可观察行为证据。"],
    quality: ["需要明确角色扮演、讨论记录和反馈证据。", "建议把评价维度写成行为化描述。"],
    actions: ["生成角色扮演脚本", "创建沟通反馈表", "整理行为证据维度"],
  },
  {
    id: "control-quality",
    type: "module",
    ...
    x: 64,
    y: 76,
    summary: "组织过程控制、质量改进、风险监测和课后复盘类资产。",
    boundary: ["强调教学中的控制逻辑与质量改进，不替代真实监管结论。", "涉及学生学习数据时需要匿名化和教师确认。"],
    quality: ["需要前测、过程记录、作业证据和复盘建议形成闭环。", "质量诊断应能转化为下一轮教学调整。"],
    actions: ["生成复盘报告", "整理学习证据", "创建质量改进清单"],
  },
  {
    ...
    x: 80,
    y: 56,
    summary: "沉淀战略选择、环境变化、创新路径和长期改进相关课程资产。",
    boundary: ["案例讨论服务课程学习，不作真实企业战略建议。", "外部政策和行业材料需要保留引用边界。"],
    quality: ["需要外部环境材料、课程案例和学生方案之间的证据链。", "评价重点是战略逻辑、可行性和风险意识。"],
    actions: ["生成战略案例包", "创建方案比较表", "检查政策材料来源"],
  },
  {
    id: "case-library",
    type: "asset",
    ...
    x: 50,
    y: 18,
    summary: "汇总教师上传材料、政策网页、课程案例和课堂资料的元数据。",
    boundary: ["浏览器版本默认记录元数据，不读取本地文件正文。", "外部网页和政策材料需要教师确认是否可面向学生分发。"],
    quality: ["优先补齐来源、授权范围、关联课程模块和使用限制。"],
    actions: ["导入课程材料", "补齐来源边界", "生成案例摘要"],
  },
  {
    id: "rubric-bank",
    type: "asset",
    ...
    x: 15,
    y: 82,
    summary: "沉淀评价维度、评分标准、反馈模板和教师确认后的量规版本。",
    boundary: ["量规可复用但不能跳过教师校准。", "正式计分前需符合本校过程性评价规则。"],
    quality: ["每个维度应有可观察证据、水平描述和改进建议。"],
    actions: ["生成评价量规", "检查评分权重", "整理反馈模板"],
  },
  {
    ...
    x: 85,
    y: 82,
    summary: "组织泛雅模拟记录、课堂活动结果、学生作业摘要和教学复盘建议。",
    boundary: ["涉及学生学习数据时必须匿名化。", "泛雅写入仍为模拟或教师确认，不做真实平台自动提交。"],
    quality: ["证据要能回到学习起点诊断、课堂调控和课后改进。"],
    actions: ["生成教学复盘", "整理共性问题", "创建下一轮调整建议"],
  },
];
...
const MANAGEMENT_COURSE_GRAPH_LINKS = [
  ...
  ["course-core", "organization-design"],
  ["course-core", "leadership-motivation"],
  ["course-core", "control-quality"],
  ["course-core", "innovation-strategy"],
  ["course-core", "case-library"],
  ["course-core", "rubric-bank"],
  ["course-core", "learning-evidence"],
  ["planning-decision", "swot-example"],
  ["case-library", "management-foundation"],
  ["case-library", "planning-decision"],
  ["rubric-bank", "swot-example"],
  ["rubric-bank", "control-quality"],
  ["learning-evidence", "control-quality"],
  ["learning-evidence", "innovation-strategy"],
];

const managementCourseGraphState = {
  selectedId: "course-core",
  activeModule: "全部",
};

function getManagementCourseNode(id) {
  return MANAGEMENT_COURSE_GRAPH_NODES.find((node) => node.id === id) || MANAGEMENT_COURSE_GRAPH_NODES[0];
}

function getManagementCourseModules() {
  return ["全部", ...Array.from(new Set(MANAGEMENT_COURSE_GRAPH_NODES.filter((node) => node.type === "module").map((node) => node.module)))];
}

function getManagementCourseAssetStats() {
  const store = currentAssetStore || loadAssets();
  const assets = getAssetWorkbenchItems();
  const withBoundary = assets.filter((asset) => asset.boundary && asset.boundary !== "待补充").length;
...
function isManagementCourseNodeFocused(node) {
  if (managementCourseGraphState.activeModule === "全部" || node.type === "core") return true;
  if (node.module === managementCourseGraphState.activeModule) return true;
  if (node.type === "asset") return true;
  const selectedModule = MANAGEMENT_COURSE_GRAPH_NODES.find(
    (item) => item.module === managementCourseGraphState.activeModule && item.type === "module",
  );
  return MANAGEMENT_COURSE_GRAPH_LINKS.some(
    ([from, to]) =>
      selectedModule &&
      ((from === selectedModule.id && to === node.id) || (to === selectedModule.id && from === node.id)),
  );
}

function renderManagementCourseToolbar() {
  const toolbar = $("#courseGraphToolbar");
  if (!toolbar) return;
  toolbar.innerHTML = getManagementCourseModules()
    .map(
      (module) => `
        <button
          type="button"
          class="${managementCourseGraphState.activeModule === module ? "is-active" : ""}"
          data-course-module-filter="${escapeHtml(module)}"
          aria-pressed="${managementCourseGraphState.activeModule === module ? "true" : "false"}"
        >
          ${escapeHtml(module)}
        </button>
      `,
    ...
    .join("");
  $$("[data-course-module-filter]", toolbar).forEach((button) => {
    button.addEventListener("click", () => {
      managementCourseGraphState.activeModule = button.dataset.courseModuleFilter || "全部";
      if (managementCourseGraphState.activeModule !== "全部") {
        const moduleNode = MANAGEMENT_COURSE_GRAPH_NODES.find((node) => node.module === managementCourseGraphState.activeModule && node.type === "module");
        if (moduleNode) managementCourseGraphState.selectedId = moduleNode.id;
      }
...
function renderManagementCourseEdges() {
  const target = $("#courseGraphEdges");
  if (!target) return;
  const nodeMap = new Map(MANAGEMENT_COURSE_GRAPH_NODES.map((node) => [node.id, node]));
  target.innerHTML = MANAGEMENT_COURSE_GRAPH_LINKS.map(([fromId, toId]) => {
    const from = nodeMap.get(fromId);
    const to = nodeMap.get(toId);
    if (!from || !to) return "";
    const selected = managementCourseGraphState.selectedId === fromId || managementCourseGraphState.selectedId === toId;
    const focused = isManagementCourseNodeFocused(from) && isManagementCourseNodeFocused(to);
    const midX = (from.x + to.x) * 5;
    const midY = (from.y + to.y) * 3.1 - 24;
    return `
      <path
        class="course-map-link ${selected ? "is-selected" : ""} ${focused ? "is-focused" : "is-dimmed"}"
        d="M ${from.x * 10} ${from.y * 6.2} Q ${midX} ${midY} ${to.x * 10} ${to.y * 6.2}"
      />
    `;
  }).join("");
}

function renderManagementCourseNodes() {
  const target = $("#courseGraphNodes");
  if (!target) return;
  target.innerHTML = MANAGEMENT_COURSE_GRAPH_NODES.map((node) => {
    const selected = managementCourseGraphState.selectedId === node.id;
    const focused = isManagementCourseNodeFocused(node);
    return `
      <button
        type="button"
        class="course-map-node is-${escapeHtml(node.type)} ${selected ? "is-selected" : ""} ${focused ? "is-focused" : "is-dimmed"}"
        style="--node-x:${node.x}; --node-y:${node.y};"
        data-course-node="${escapeHtml(node.id)}"
        aria-pressed="${selected ? "true" : "false"}"
      >
        <span>${escapeHtml(node.title)}</span>
        <small>${escapeHtml(node.module)}</small>
      </button>
    `;
  }).join("");
  $$("[data-course-node]", target).forEach((button) => {
    button.addEventListener("click", () => {
      managementCourseGraphState.selectedId = button.dataset.courseNode || "course-core";
      renderManagementCourseGraph();
    });
  });
}

function renderManagementCoursePanels() {
  const node = getManagementCourseNode(managementCourseGraphState.selectedId);
  const stats = getManagementCourseAssetStats();
  const title = $("#courseNodeOverviewTitle");
  const overview = $("#courseNodeOverview");
  const boundaryTitle = $("#courseBoundaryTitle");
  const boundary = $("#courseBoundaryPanel");
  const diagnosticsTitle = $("#courseDiagnosticsTitle");
  const diagnostics = $("#courseDiagnosticsPanel");
  const actionsTitle = $("#courseActionsTitle");
  const actions = $("#courseActionsPanel");
  const status = $("#courseGraphStatus");

  if (title) title.textContent = node.title;
  if (overview) {
    overview.innerHTML = `
      <p>${escapeHtml(node.summary)}</p>
      <dl class="course-panel-facts">
        <div><dt>所属模块</dt><dd>${escapeHtml(node.module)}</dd></div>
        <div><dt>节点类型</dt><dd>${node.type === "core" ? "课程主干" : node.type === "module" ? "课程模块" : node.type === "example" ? "示例节点" : "资产节点"}</dd></div>
        <div><dt>图谱位置</dt><dd>${node.type === "example" ? "模块下子节点" : node.type === "asset" ? "课程资产支撑层" : "课程结构层"}</dd></div>
      </dl>
    `;
  }
  if (boundaryTitle) boundaryTitle.textContent = `${node.module}边界`;
  if (boundary) {
    boundary.innerHTML = `
      <ul class="course-panel-list">
        ${(node.boundary || []).map((item) => `<li>${escapeHtml(item)}</li>`).join("")}
      </ul>
      ${
        node.id === "swot-example"
          ? `<p class="course-boundary-note">已限定：SWOT 只属于“计划与决策”模块的示例节点。</p>`
          : `<p class="course-boundary-note">当前节点不改变课程主干，所有资产复用都需要回到管理学原理模块边界。</p>`
      }
    `;
  }
  if (diagnosticsTitle) diagnosticsTitle.textContent = `${node.module}诊断`;
  if (diagnostics) {
  ...
  if (actions) {
    ...
    $$("[data-course-node-action]", actions).forEach((button) => {
      button.addEventListener("click", () => {
        const action = button.dataset.courseNodeAction || "复制节点上下文";
        copyText(
          [
            `课程图谱节点：${node.title}`,
            `所属模块：${node.module}`,
            `建议动作：${action}`,
            `节点说明：${node.summary}`,
            `边界说明：${(node.boundary || []).join("；")}`,
          ].join("\n"),
        );
        showToast(`已复制“${action}”上下文`);
      });
    });
  }
  if (status) {
    status.innerHTML = `
      <span>当前选中：${escapeHtml(node.title)}</span>
      <span>模块：${escapeHtml(node.module)}</span>
      <span>资产：${stats.total} 项</span>
      <span>边界：SWOT 仅为“计划与决策”模块示例</span>
    `;
  }
}
...
function renderManagementCourseGraph() {
  ...
  renderManagementCourseEdges();
  renderManagementCourseNodes();
  renderManagementCoursePanels();
}

function initManagementCourseGraphPage() {
...
function initAssetsPage() {
  if (initManagementCourseGraphPage()) return;
  if (!$("#assetList")) return;
  loadAssets();
  importTrainingAndPracticeAssets();
  renderAssetOverview();
