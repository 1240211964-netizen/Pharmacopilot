...
const TRAINING_STEP_RUBRICS = [
  makeTrainingStepRubric(1, "课程定位与专业价值", "阶段 A：教学起点与课程定标", "课程定位成熟度诊断", "评价这节课为什么要教，以及它在药事管理专业培养中的位置是否清楚。", [
    ...
    rubricDimension("topic_boundary_accuracy", "主题边界准确性", "是否避免把 SWOT 讲成泛泛管理学概念，而是限定在药事管理本科课堂案例中。", "主题边界是否具体到一个明确场景？", "将主题限定到药事管理本科课堂中的 SWOT 分析教学。"),
  ...
  makeTrainingStepRubric(2, "课程主题边界与核心问题", "阶段 A：教学起点与课程定标", "核心问题清晰度诊断", "评价本节课是否围绕一个清晰、真实、可讨论的核心问题展开。", [
    ...
    rubricDimension("pharmacy_context_embedding", "药学情境嵌入度", "是否把主题放入药事管理本科课堂案例场景。", "学生能否看到该问题与药事管理实践的关系？", "补充政策材料、机构管理、公共卫生管理和监管要求等决策背景。"),
  ...
  makeTrainingStepRubric(7, "药事管理真实情境建构", "阶段 B：目标设计与内容转化", "真实情境质量诊断", "评价抽象理论是否被转化为真实药事管理问题。", [
    rubricDimension("situational_authenticity", "情境真实性", "是否来自真实或高仿真的药事管理问题。", "情境是否像真实药事管理工作中会遇到的问题？", "补充真实业务背景，例如药品政策、机构管理、公共卫生管理或监管要求。"),
...
const routeTrainingNodes = [
  {
    ...
    x: 9,
    y: 78,
    goal: "明确《管理学原理》在药事管理本科培养中的作用。",
    task: "用一句话说明本课程如何支持学生形成管理决策能力。",
    output: "课程定位说明。",
    rubric: "是否清楚说明课程、学生、能力三者关系。",
    theory: "OBE 成果导向教育。",
  },
  {
    id: 2,
    ...
    x: 18,
    y: 66,
    goal: "识别药事管理本科生的知识基础、学习困难和职业期待。",
    task: "整理学生已有基础与可能的 SWOT 学习障碍。",
    output: "学生画像卡。",
    rubric: "是否包含知识基础、认知难点、应用场景。",
    theory: "学习者中心教学。",
  },
  {
    id: 3,
    ...
    x: 28,
    y: 74,
    goal: "将 SWOT 分析教学目标从知识记忆提升到管理决策能力。",
    task: "按照认知层级写出知识、能力和价值目标。",
    output: "三层教学目标。",
    rubric: "目标是否具体、可观察、可评价。",
    theory: "Bloom 认知目标分类。",
  },
  {
    id: 4,
    ...
    x: 35,
    y: 58,
    goal: "说明为什么药事管理学生需要掌握 SWOT 分析。",
    task: "结合药品政策、医药企业、医院药学或公共卫生管理场景说明 SWOT 的价值。",
    output: "教学意义说明。",
    rubric: "是否体现药事管理专业语境。",
    theory: "情境学习理论。",
  },
  {
    ...
    x: 47,
    y: 64,
    goal: "选择适合 SWOT 分析的药事管理案例。",
    task: "构建一个真实、适度复杂、适合本科课堂讨论的案例。",
    output: "SWOT 案例简表。",
    rubric: "案例是否具有管理决策张力。",
    theory: "案例教学法。",
  },
  {
    ...
    x: 56,
    y: 48,
    goal: "设计能激发学生问题意识的课堂导入。",
    task: "写出 3 分钟导入问题和教师引导语。",
    output: "课堂导入脚本。",
    rubric: "是否能引出管理问题而非单纯背景介绍。",
    theory: "问题导向学习。",
  },
  {
    id: 7,
    ...
    x: 66,
    y: 55,
    goal: "清楚讲解 SWOT 中 S、W、O、T 的边界。",
    task: "设计一张概念区分表，避免学生混淆内部与外部因素。",
    output: "SWOT 概念解释表。",
    rubric: "概念是否准确，边界是否清晰。",
    theory: "概念教学。",
  },
  {
    id: 8,
    ...
    x: 74,
    y: 42,
    goal: "让学生在小组中完成 SWOT 识别。",
    task: "设计小组角色、任务分工和讨论流程。",
    output: "小组任务单。",
    rubric: "任务是否清楚、时间是否可控、分工是否合理。",
    theory: "合作学习。",
  },
  {
    id: 9,
    ...
    x: 85,
    y: 50,
    goal: "让学生基于证据而非主观印象完成分析。",
    task: "准备政策、市场、机构或管理情境材料。",
    output: "课堂证据包。",
    rubric: "材料是否真实、简洁、可用于判断。",
    theory: "证据导向教学。",
  },
  {
    id: 10,
    ...
    x: 78,
    y: 29,
    goal: "指导学生形成规范的 SWOT 矩阵。",
    task: "设计学生填写模板和教师示范样例。",
    output: "SWOT 矩阵模板。",
    rubric: "矩阵是否结构完整、分类准确。",
    theory: "支架式教学。",
  },
  {
    id: 11,
    ...
    x: 64,
    y: 27,
    goal: "引导学生从因素罗列进入管理决策。",
    task: "设计一个需要基于 SWOT 做出选择的决策问题。",
    output: "决策问题卡。",
    rubric: "是否从分析走向判断和选择。",
    theory: "高阶思维培养。",
  },
  {
    ...
    y: 35,
    goal: "通过追问促进学生深化分析。",
    task: "设计至少 5 个追问，包括证据追问、因果追问、权衡追问。",
    output: "教师追问清单。",
    rubric: "追问是否促进分析、评价和创造。",
    theory: "苏格拉底式提问。",
  },
  {
    id: 13,
  ...
  },
  {
    ...
    x: 30,
    y: 34,
    goal: "控制导入、讲解、讨论、展示和总结的时间。",
    task: "设计 45 分钟或 90 分钟课堂节奏表。",
    output: "课堂时间轴。",
    rubric: "时间安排是否合理，活动切换是否顺畅。",
    theory: "教学事件设计。",
  },
  {
    id: 15,
    ...
    x: 19,
    y: 27,
    goal: "让学生表达并比较不同小组的分析结果。",
    task: "设计展示规则、发言要求和同伴提问方式。",
    output: "小组展示规则。",
    rubric: "是否促进表达、比较和反思。",
    theory: "同伴学习。",
  },
  {
    ...
    x: 13,
    y: 43,
    goal: "建立可评价的 SWOT 学习表现标准。",
    task: "设计包含概念准确性、证据使用、逻辑分析、决策建议的 rubric。",
    output: "SWOT 分析评价量表。",
    rubric: "维度是否清晰，等级描述是否可区分。",
    theory: "Rubric 评价。",
  },
  {
    id: 17,
  ...
  },
  {
    ...
    x: 40,
    y: 45,
    goal: "整理课堂表现、作业结果和学生反馈数据。",
    task: "归纳学生在 SWOT 学习中的共性问题。",
    output: "教学数据摘要。",
    rubric: "是否基于数据而非印象总结。",
    theory: "学习分析。",
  },
  {
    ...
    x: 55,
    y: 20,
    goal: "基于数据反思教学目标、活动和评价是否一致。",
    task: "写出本节课的优点、问题和改进方向。",
    output: "教学反思报告。",
    rubric: "是否形成目标—活动—评价的一致性分析。",
    theory: "反思性教学。",
  },
  {
    id: 20,
    ...
    y: 18,
    goal: "形成可执行的课程迭代方案。",
    task: "提出下一轮 SWOT 教学的具体改进措施。",
    output: "教学改进计划。",
    rubric: "措施是否具体、可执行、可复盘。",
    theory: "持续改进闭环。",
  },
];
...
const routeStageDefinitions = [
  {
    id: 1,
    title: "明确教学目的和意义",
    copy: "理解课程定位、学生画像、管理学核心能力和 SWOT 分析的教学价值。",
  },
  {
    id: 2,
    title: "完成 20 环节教学实践训练",
    copy: "从教学目标、案例设计、课堂活动、评价量表到学生反馈，逐步完成教学设计与实施。",
  },
  {
    id: 3,
    title: "总结平台数据和教学经验",
    copy: "基于教学过程数据、学生表现和评价结果，形成教学反思与改进方案。",
  },
];
...
function initTeachingNavigationPage() {
  ...
  const currentNode = routeTrainingNodes.find((node) => node.status === "current");
  selectedRouteNodeId = currentNode?.id || routeTrainingNodes[0].id;
  renderTeachingNavigationPage();
  document.addEventListener("click", (event) => {
    if (event.target.closest("[data-close-training-modal]")) closeTrainingRouteModal();
  });
  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape") closeTrainingRouteModal();
  });
}
...
function renderTeachingHeroStatus() {
  const completed = routeTrainingNodes.filter((node) => node.status === "completed").length;
  const current = getRouteNode();
  const progress = $("#teachingRouteProgress");
  const stage = $("#teachingRouteStage");
  const task = $("#teachingRouteNextTask");
  if (progress) progress.textContent = `${completed + 1} / ${routeTrainingNodes.length}`;
...
function getRouteCurrentPhaseLabel(node) {
  if (!node) return "课程设计路径";
  if (node.id <= 4) return "课程理解与目标校准";
  if (node.id <= 6) return "案例入口与课堂导入";
  if (node.id <= 12) return "活动任务与证据支架";
  if (node.id <= 17) return "课堂组织与评价设计";
  return "数据复盘与持续改进";
}

function getRouteStageStatus(nodes) {
  if (nodes.every((node) => node.status === "completed")) return "已完成";
  if (nodes.some((node) => node.status === "current")) return "进行中";
  return "待解锁";
}
  ...
  svg.innerHTML = `
    ...
    ${pairs
      .map(([startNode, endNode]) => {
        const active = startNode.status === "completed";
        return `
          <path
...
function renderStageOverview() {
  ...
  container.innerHTML = routeStageDefinitions
    .map((stage) => {
      ...
      return `
        <article class="teaching-stage-card stage-status-${status === "已完成" ? "complete" : status === "进行中" ? "current" : "locked"}">
...
function renderNewTeacherMap() {
  ...
  if (statsContainer) {
    ...
    statsContainer.innerHTML = statusOrder
      .map((status) => {
        const count = routeTrainingNodes.filter((node) => node.status === status).length;
        return `
...
function renderMapNode(node) {
  ...
  const statusLabel = routeStatusLabels[node.status] || node.status;
  const icon = node.status === "completed" ? "✓" : node.status === "locked" ? "锁" : String(node.id);
  return `
    <button
      class="route-node route-node-${node.status} ${selected ? "is-selected" : ""}"
      type="button"
...
function renderNodeDetailPanel() {
  ...
  container.innerHTML = `
    <article class="route-node-detail-card">
      <div class="route-detail-kicker">
        <span>节点 ${String(node.id).padStart(2, "0")}</span>
        <strong class="route-detail-status status-${node.status}">${escapeHtml(statusLabel)}</strong>
      </div>
      <h3>${escapeHtml(node.title)}</h3>
      <p>${escapeHtml(getRouteStageTitle(node.stage))}</p>
      <dl class="route-detail-list">
        <div><dt>训练目标</dt><dd>${escapeHtml(node.goal)}</dd></div>
        <div><dt>教师要完成的任务</dt><dd>${escapeHtml(node.task)}</dd></div>
        <div><dt>产出物</dt><dd>${escapeHtml(node.output)}</dd></div>
        <div id="nodeRubricDetail" class="${routeRubricExpanded ? "is-rubric-focused" : ""}"><dt>评价维度</dt><dd>${escapeHtml(node.rubric)}</dd></div>
        <div><dt>理论依据</dt><dd>${escapeHtml(node.theory)}</dd></div>
      </dl>
      <div class="route-detail-actions">
        <button class="primary-action ${locked ? "is-disabled" : ""}" data-route-action="enter" data-route-locked="${locked ? "true" : "false"}" type="button">进入训练</button>
        <button class="secondary-action" data-route-action="rubric" type="button">查看评价标准</button>
        <button class="secondary-action" data-route-action="material" type="button">生成教学材料</button>
      </div>
    </article>
  `;
  $("[data-route-action='enter']", container)?.addEventListener("click", () => {
    if (locked) {
      showToast("请先完成前序环节，再进入该训练任务");
      return;
    }
    openTrainingRouteModal(node);
  });
  $("[data-route-action='rubric']", container)?.addEventListener("click", () => {
    routeRubricExpanded = true;
    renderNodeDetailPanel();
    $("#nodeRubricDetail")?.scrollIntoView({ behavior: "smooth", block: "nearest" });
  });
  $("[data-route-action='material']", container)?.addEventListener("click", () => {
    showToast("后续将连接课程智能体生成教案、课件、任务单和评价量表。");
  });
}
...
function renderProgressSummary() {
  ...
  if (!container) return;
  const current = getRouteNode(routeTrainingNodes.find((node) => node.status === "current")?.id);
  const completed = routeTrainingNodes.filter((node) => node.status === "completed").length;
  const stageTwoNodes = routeTrainingNodes.filter((node) => node.stage === 2);
  const stageTwoReady = stageTwoNodes.filter((node) => node.status === "completed" || node.status === "current").length;
  const reviewReady = routeTrainingNodes.filter((node) => node.stage === 3 && node.status !== "locked").length;
  const summaries = [
    { label: "教学设计完成度", value: `${Math.round((completed / routeTrainingNodes.length) * 100)}%`, copy: "已完成课程定位、学生画像、目标设定、教学意义和案例选择。" },
    { label: "课堂活动准备度", value: `${Math.round((stageTwoReady / stageTwoNodes.length) * 100)}%`, copy: "当前正在完善课堂导入，后续进入概念讲解、任务单、证据包与节奏设计。" },
    { label: "评价闭环完整度", value: `${Math.round((reviewReady / 3) * 100)}%`, copy: "评价量表、学生反馈、教学数据整理和复盘计划尚待建立。" },
  ];
  container.innerHTML = `
    ${summaries
      .map(
    ...
    <article class="route-progress-card route-next-advice">
      ...
      <p>你已经完成课程定位、学生画像、目标设定、教学意义和案例选择。建议继续完善“${escapeHtml(current.title)}”，检查导入问题是否能把学生从背景描述引向管理判断。</p>
    </article>
  `;
}
