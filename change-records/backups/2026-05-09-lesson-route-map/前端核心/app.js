...
function renderTeachingHeroStatus() {
  ...
  if (stage) stage.textContent = getRouteStageTitle(current.stage);
...
function renderNewTeacherMap() {
  ...
  const completedPath = $("#teachingRouteCompletedPath");
  if (!container) return;
  const currentNode = routeTrainingNodes.find((node) => node.status === "current");
  const progressRatio = currentNode ? Math.max(0.1, (currentNode.id - 0.5) / routeTrainingNodes.length) : 0.05;
  if (completedPath) completedPath.style.strokeDasharray = `${Math.round(progressRatio * 100)} 100`;
...
function renderMapNode(node) {
  ...
  const icon = node.status === "completed" ? "✓" : node.status === "locked" ? "⌕" : String(node.id);
  return `
    <button
...
function renderProgressSummary() {
  ...
  const summaries = [
    { label: "教学设计完成度", value: "30%", copy: "已完成课程定位、学生画像、目标设定与案例入口准备。" },
    { label: "课堂活动准备度", value: "22%", copy: "当前正在完善课堂导入，后续进入任务单、证据包与节奏设计。" },
    { label: "评价闭环完整度", value: "12%", copy: "评价量表、学生反馈与复盘计划尚未完成。" },
  ];
  container.innerHTML = `
    ${summaries
      .map(
    ...
    <article class="route-progress-card route-next-advice">
      ...
      <p>你已经完成课程定位与教学目标设定，建议继续完善 SWOT 案例任务单，并补充评价量表。</p>
    </article>
  `;
}
